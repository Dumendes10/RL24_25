...
# Custom evaluate_policy function that handles different algorithms
    ...
    for _ in range(eval_episodes):
        ...
        while not (done or truncated):
            ...
            if hasattr(policy, '__class__') and policy.__class__.__name__ == 'SAC':
                action = policy.select_action(np.array(obs))  # SAC doesn't accept noise
...
def train_sac_with_params(params, env_name=ENV_NAME, max_timesteps=30000, eval_freq=5000, 
    ...
    for t in range(1, max_timesteps + 1):
        ...
        if done:
            ...
            if use_her and len(episode_experiences) > 0:
                replay_buffer.add_her_samples(episode_experiences, goal_selection_strategy="final")
    ...
    result = {
        'algorithm': 'SAC_HER' if use_her else 'SAC',
        'params': params,
    ...
    if save_models and final_success_rate > 0.5:
        ...
        model_dir = f"{save_dir}/sac{'_her' if use_her else ''}_{timestamp}_{final_success_rate:.2f}"
...
def train_td3_with_params(params, env_name=ENV_NAME, max_timesteps=30000, eval_freq=5000, 
    ...
    # Store training data and parameters
    result = {
        'algorithm': 'TD3_HER' if use_her else 'TD3',
        'params': params,
        'rewards': np.array(training_rewards),
        'success_rates': np.array(success_rates),
        'evaluations': np.array(evaluations),
    ...
    if save_models and final_success_rate > 0.5:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_dir = f"{save_dir}/td3{'_her' if use_her else ''}_{timestamp}_{final_success_rate:.2f}"
        os.makedirs(model_dir, exist_ok=True)
        policy.save(f"{model_dir}/model")
...
def generate_sac_params():
    ...
    """
    params = {
        # Simpler networks might learn faster for this task
        'hidden_dim': np.random.choice([64, 128, 256]),
        
        # Higher learning rates to accelerate learning
        'lr': np.random.choice([3e-4, 5e-4, 7e-4, 1e-3, 2e-3]),
        
        # Lower gamma values for more immediate rewards
        'gamma': np.random.uniform(0.9, 0.98),
        
        # More emphasis on faster target network updates
        'tau': np.random.choice([0.005, 0.01, 0.02, 0.05]),
        
        # Lower alpha values to reduce excessive exploration
        'alpha': np.random.uniform(0.01, 0.2),
        
        # More weight on automatic tuning which can adapt during training
        'automatic_entropy_tuning': np.random.choice([True, True, False]),
        
        # Smaller batch sizes for faster updates
        'batch_size': np.random.choice([32, 64, 128]),
        
        # Smaller buffer sizes for this simpler task
...
def generate_td3_params():
    ...
    """
    params = {
        # Simpler networks for faster learning
        'hidden_dim': np.random.choice([64, 128, 256]),
        
        # Lower gamma values for more immediate rewards
        'gamma': np.random.uniform(0.9, 0.98),
        
        # Faster target network updates
        'tau': np.random.choice([0.005, 0.01, 0.02, 0.05]),
        
        # Lower policy noise for more stable learning
        ...
        # Lower exploration noise for more focused exploration
...
def run_random_search(n_trials=5, max_timesteps=30000, save_dir="./results/random_search", use_her=False, algorithms=None):
    """
    ...
    if 'sac' in algorithms:
        print("\n==== Running Random Search for SAC" + (" with HER" if use_her else "") + " ====")
        for i in range(n_trials):
            ...
            np.save(f"{save_dir}/sac{'_her' if use_her else ''}_results.npy", sac_results)
            
            print(f"Trial completed. Final success rate: {result['final_success_rate']:.3f}")
    
    # Run random search for TD3
    if 'td3' in algorithms:
        print("\n==== Running Random Search for TD3" + (" with HER" if use_her else "") + " ====")
        for i in range(n_trials):
            ...
            # Save partial results after each trial
            np.save(f"{save_dir}/td3{'_her' if use_her else ''}_results.npy", td3_results)
            
            print(f"Trial completed. Final success rate: {result['final_success_rate']:.3f}")
    
    # Analyze results
    if sac_results or td3_results:
        analyze_results(sac_results, td3_results, save_dir, use_her)
...
def analyze_results(sac_results, td3_results, save_dir, use_her=False):
    """
    ...
    # Plot comparison of best agents if both algorithms were run
    if sac_results and td3_results:
        plt.figure(figsize=(18, 6))
        
        # Plot success rates
        plt.subplot(1, 3, 1)
        plt.plot(best_sac['success_rates'], label='SAC' + suffix)
        plt.plot(best_td3['success_rates'], label='TD3' + suffix)
        ...
        if len(best_sac['episode_lengths']) >= window_size:
            ...
            plt.plot(sac_lengths_smooth, label='SAC' + suffix)
        else:
            plt.plot(best_sac['episode_lengths'], label='SAC' + suffix)
            
        if len(best_td3['episode_lengths']) >= window_size:
            td3_lengths_smooth = np.convolve(best_td3['episode_lengths'], weights, mode='valid')
            plt.plot(td3_lengths_smooth, label='TD3' + suffix)
        else:
            plt.plot(best_td3['episode_lengths'], label='TD3' + suffix)
